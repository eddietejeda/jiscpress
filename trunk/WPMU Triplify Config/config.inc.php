...
// Establish blog public status
$get_blog_status_sql = "SELECT option_value FROM {$table_prefix}options WHERE option_name = 'blog_public'";
$get_blog_status = mysql_query($get_blog_status_sql);
if($get_blog_status !== FALSE){
	$result = mysql_fetch_assoc($get_blog_status);
	if($result['option_value'] == '0'){
		die("This blog is not public therefore no Triple will be created");
	}
	mysql_close($wpmu_connect);
} else {
	die("Mysql error when trying to get the blog public status:<br/><code>".mysql_error()."</code>");
}
...
$triplify['queries']=array(
	...
	'comment'=>array(
		/* all comments */
		"SELECT {$table_prefix}comments.comment_ID id, {$table_prefix}comments.comment_post_id 'sioc:reply_of', {$table_prefix}comments.comment_author AS 'foaf:name',
		SHA1(CONCAT('mailto:',{$table_prefix}comments.comment_author_email)) 'foaf:mbox_sha1sum',
		{$table_prefix}comments.comment_author_url 'foaf:homepage', {$table_prefix}comments.comment_date AS'dcterms:created',
		{$table_prefix}comments.comment_content 'sioc:content', {$table_prefix}comments.comment_karma, {$table_prefix}comments.comment_type
		FROM {$table_prefix}comments, {$table_prefix}posts WHERE {$table_prefix}comments.comment_approved='1' AND {$table_prefix}comments.comment_post_id = {$table_prefix}posts.ID AND {$table_prefix}posts.post_status = 'publish'"
	)
);
